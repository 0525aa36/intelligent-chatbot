# Backend Developer 사전과제 (JVM) - Intelligent Chatbot Service

안녕하세요 지원자님!
본 문서는 Backend Developer 사전과제에 대한 구현 내용과 기술적 의사결정 과정을 담고 있습니다.

---

## 1. 과제를 어떻게 분석하셨나요?

과제를 받자마자 **주도적으로 문제를 정의**하고 다음과 같은 핵심 질문을 스스로에게 던졌습니다: "3시간 안에 무엇을 반드시 보여줘야 하는가?" 그리고 "확장 가능한 아키텍처란 무엇을 의미하는가?"에 대한 깊은 고민을 시작했습니다.

### 1.1 문제 정의 및 우선순위 설정

**핵심 질문과 답변:**
1.  **"3시간 안에 무엇을 반드시 보여줘야 하는가?"**
    *   **답변**: 고객사가 AI API를 통해 실제 질문을 던지고 응답을 받는 것을 확인할 수 있어야 합니다. 즉, **AI 연동을 통한 대화 생성 기능**이 시연의 핵심입니다.
...
3.  **"시연에서 가장 중요한 것은?"**
    *   **답변**: 안정성입니다. 아무리 고급 기능이 많아도 기본 기능이 불안정하면 시연은 실패합니다. 따라서 **기본 기능의 완벽한 작동**을 최우선으로 두었습니다.
...
*   **아키텍처 설계 - 인터페이스 추상화 (`AiClient`)**:
    *   **설계 의도**: `AiClient` 인터페이스를 정의하고, `OpenAiClientImpl` (실제로는 Gemini 구현체)이 이를 구현하도록 했습니다.
    *   **코드 예시**:
        ```kotlin
        interface AiClient {
...
*   **3시간 초과**: MVP(Minimum Viable Product)를 우선 구현하고, 문서화는 마지막 30분에 집중하는 전략을 세웠습니다.
...
*   **보일러플레이트 코드 생성 (약 30% 시간 절약)**:
    *   **프롬프트 예시**: "Spring Boot + Kotlin으로 JWT 인증 필터 구현해줘. SecurityConfig와 연동되고, Bearer 토큰 추출 로직 포함"
    *   **활용**: `JwtAuthenticationFilter.kt`의 기본 구조를 빠르게 생성하여, 저는 비즈니스 로직 검토 및 예외 처리와 같은 핵심 작업에 집중할 수 있었습니다.
*   **테스트 코드 작성 (품질 보증)**:
...
## 4. 시연을 위한 HTTP Client 검증 (선택 사항: 스크린샷 포함 권장)

프로젝트의 모든 API 엔드포인트는 `api-test.http` 파일을 통해 직접 테스트하고 검증할 수 있습니다. 이 파일은 IntelliJ IDEA Ultimate 또는 VS Code의 REST Client 확장 프로그램에서 바로 실행 가능합니다.

**권장 사항**: 각 섹션의 설명과 함께 `api-test.http` 파일을 실행한 후의 HTTP Client 캡처 화면(요청 및 응답)을 첨부하시면, 구현된 기능과 검증 과정을 더욱 명확하게 보여줄 수 있습니다.
